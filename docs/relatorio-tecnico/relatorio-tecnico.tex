...
Este relatório documenta o desenvolvimento do \textbf{Catálogo Simples de Filmes}, projeto
...
conduzida pela metodologia de Aprendizagem Baseada em Projetos (ABP). O enunciado da disciplina
propõe, de forma genérica, um ``Catálogo Simples de Livros ou Filmes'': uma aplicação web capaz
de catalogar e consultar itens de mídia, com atributos mínimos de título, autor/diretor, ano de
publicação/lançamento, gênero e sinopse.

Este projeto adota um recorte específico dessa proposta genérica: um catálogo focado
\textbf{exclusivamente em filmes}, com a entidade central \texttt{Filme}. A escolha da temática
de cinema, em vez de um catálogo multi-mídia (livros, séries e filmes ao mesmo tempo), foi uma
...
O objetivo principal é aplicar, na prática, os conhecimentos de Programação Orientada a Objetos
em Java a um sistema web completo, contemplando:
...
Além do escopo mandatório do enunciado, o autor optou por implementar duas extensões opcionais
...
com o framework Bootstrap 5, incluindo modo escuro alternável e efeitos visuais. Essas duas
extensões são descritas em detalhe nas Seções~\ref{sec:desenvolvimento} e~\ref{sec:resultados}.

\subsection{Justificativa}

Ainda que o caráter do trabalho seja predominantemente acadêmico, a justificativa do projeto é
dupla. Do ponto de vista pedagógico, o desenvolvimento de um CRUD completo com busca --
exatamente o escopo descrito no enunciado -- exercita de forma concentrada os pilares do
desenvolvimento web em Java (Servlets como controladores, JSP como camada de visão, DAO como
camada de acesso a dados via JDBC) e, de forma deliberada, expõe o autor a três
...
de vista prático, o resultado é uma ferramenta de uso pessoal genuinamente útil: um catálogo de
filmes assistidos/a assistir, enriquecido com dados públicos do TMDB.
...
O desenvolvimento deste projeto segue integralmente a metodologia de Aprendizagem Baseada em
...
1991). No contexto da Ciência da Computação, essas características se traduzem diretamente no
desenvolvimento de um sistema funcional, do levantamento de requisitos até a entrega documentada,
exercitando tanto habilidades técnicas quanto competências do século XXI -- pensamento crítico,
criatividade, colaboração e comunicação (TRILLING; FADEL, 2009).

\subsection{Abordagem iterativa e processo interno de especificação}

O desenvolvimento seguiu uma abordagem \textbf{iterativa}, dividida em pequenos marcos (ciclos ou
...
(integração TMDB e reestilização de interface). Cada ciclo seguiu internamente um processo de
...
critérios de aceite e considerações de segurança. Cada especificação só avançou de fase --
Rascunho, Em Revisão, Aprovada, Implementada, Concluída -- quando os critérios daquela fase
foram integralmente satisfeitos, incluindo uma revisão de segurança e testes automatizados antes
de ser considerada concluída. Esse processo interno de especificação, documentado em
...
A stack tecnológica adotada é a exigida pelo enunciado da disciplina, sem uso de \textit{frameworks}
...
Como extensão opcional, dois casos de uso adicionais foram implementados: \textbf{Descobrir
Filmes Populares (TMDB)}, que exibe um carrossel de filmes populares consultados em tempo real na
API do TMDB, com um botão \textbf{Importar Filme do TMDB} que insere o filme escolhido
diretamente no catálogo local; e \textbf{Buscar Filme no TMDB} dentro do próprio formulário de
cadastro, que pré-preenche os campos a partir de um resultado escolhido pelo usuário. Em ambos
os casos, uma eventual indisponibilidade da API do TMDB nunca compromete o cadastro manual nem
o restante do catálogo -- uma decisão de design registrada explicitamente na especificação da
integração.
...
refletindo a decisão de projeto de que um cadastro mínimo (só o título) já é válido. As colunas
\texttt{capa\_url} e \texttt{nota\_tmdb} foram adicionadas em um segundo momento, via
...
Duas decisões de arquitetura reduziram duplicação de código, no espírito da terceira
situação-problema do enunciado (refatoração e orientação a objetos): (a) a validação de entrada
foi extraída para a classe de serviço \texttt{FilmeValidator}, compartilhada entre o cadastro e a
edição, em vez de duplicada em cada Servlet; e (b) a funcionalidade de busca não ganhou uma tela
separada -- ela estende \texttt{ListarFilmesServlet}/\texttt{listarFilmes.jsp} com um parâmetro
opcional \texttt{termo}, já que conceitualmente é a mesma listagem, apenas filtrada.
...
(mesmo título e ano de lançamento já cadastrados) adicionada em uma iteração posterior do
projeto:
...
Note-se, no trecho acima, o padrão repetido em toda a aplicação: (1) os dados de entrada passam
...
enunciado (SQL Injection ao concatenar campos de busca). A implementação em
\texttt{FilmeDAOImpl} é a seguinte:
...
O ponto central desta implementação, e a razão pela qual foi escolhida como uma das duas
...
exceções adotado em toda a camada de acesso a dados do projeto.

\subsection{Extensões opcionais: integração TMDB e reestilização com Bootstrap}

Além do escopo mandatório, o projeto implementa duas extensões opcionais mencionadas no
enunciado da disciplina como possíveis, mas não obrigatórias. A primeira, integração com a API
pública do TMDB (pacote \texttt{com.catalogo.tmdb}), permite: (a) descobrir filmes populares em
um carrossel, com importação direta ao catálogo local; e (b) buscar um filme por título dentro do
...
Esta seção apresenta capturas de tela reais da aplicação em execução localmente, demonstrando as
funcionalidades mandatórias do CRUD/busca e as duas extensões opcionais descritas na
Seção~\ref{sec:desenvolvimento}.
...
A segurança foi tratada como parte integral do desenvolvimento (princípio de \textit{Secure by
Design}, McGRAW, 2006), com uma revisão de segurança dedicada, seguindo um checklist fixo,
executada ao final de cada funcionalidade -- \textbf{antes} de qualquer especificação ser
considerada concluída -- e uma trilha independente de teste de penetração (\textit{pentest})
\textit{blackbox}, escopada pelo OWASP Top 10:2021, executada após a conclusão de todas as
funcionalidades mandatórias.

\subsection{Revisão de segurança por funcionalidade}

Todas as especificações do projeto (cadastro, listagem, detalhe, edição, exclusão, busca,
integração TMDB e reestilização de interface) passaram por uma revisão de segurança formal,
cobrindo cinco categorias fixas: SQL Injection, tratamento de exceções, validação de dados, XSS
...
Todas as revisões resultaram em aprovação integral (5/5 itens, com alguns itens marcados
\textit{N/A} quando não aplicáveis, como validação de dados em funcionalidades somente-leitura).
As poucas observações registradas durante essas revisões foram classificadas como não
bloqueantes -- lacunas de robustez defensiva, não vulnerabilidades de segurança propriamente
ditas (por exemplo, ausência de validação de faixa de valores em um campo interno que nunca é
editado diretamente pelo usuário) -- e ficaram documentadas nas especificações correspondentes
para eventual correção futura.

\subsection{Achados do pentest \textit{blackbox} (Nível 3 -- trilha de segurança)}

Após a conclusão de todas as funcionalidades mandatórias, uma trilha independente de teste de
penetração \textit{blackbox}, escopada pelo OWASP Top 10:2021, foi executada contra a aplicação
em execução, sob a regra inegociável de \textbf{reportar, nunca explorar}. Dois achados foram
registrados, ambos já triados e \textbf{encerrados}:
...
\begin{itemize}
    \item \textbf{Ausência de controle de acesso e de proteção CSRF} (severidade Média,
        ...
        confirmação do lado do cliente). Esse indício já havia sido identificado e formalmente
        aceito como risco durante as revisões de segurança de cada especificação individual; o
        pentest \textit{blackbox} apenas confirmou, de fora, o que já era conhecido. Status
        final: \textbf{Risco Aceito} -- reavaliação recomendada caso uma extensão de
        autenticação venha a ser implementada no futuro;
    \item \textbf{Ausência de cabeçalhos HTTP de segurança} (severidade Baixa, OWASP A05:2021 --
        ...
        \texttt{SameSite} explícito no cookie de sessão. Este achado foi efetivamente
        \textbf{corrigido}: foi implementado um novo filtro de \textit{servlet},
        \texttt{CabecalhosSegurancaFilter}, que adiciona os quatro primeiros cabeçalhos a toda
        resposta HTTP (com uma \textit{Content Security Policy} restrita aos domínios
        efetivamente usados pela aplicação -- CDN do Bootstrap e imagens do TMDB), e o cookie de
        sessão passou a ser configurado com \texttt{SameSite=Lax} via
        \texttt{META-INF/context.xml} do Tomcat. A correção foi revalidada (\texttt{mvn test},
        inspeção dos cabeçalhos HTTP reais e confirmação de que nenhuma funcionalidade existente
        foi quebrada). Status final: \textbf{Corrigido}.
\end{itemize}

\subsection{Cobertura de testes}

A camada de acesso a dados (\texttt{FilmeDAOImpl}) conta com uma suíte de 20 testes JUnit 5
(\texttt{src/test/java/com/catalogo/dao/FilmeDAOTest.java}), executados de fato (\texttt{mvn test})
contra um banco MariaDB real, cobrindo todos os métodos de \texttt{FilmeDAO} -- incluindo casos
de sucesso, ausência de correspondência, e um teste específico de sondagem com aspas simples
(\texttt{buscarPorTituloOuDiretor\_naoDeveLancarExcecao\_comAspasSimples}) para confirmar,
empiricamente, a ausência de vulnerabilidade a SQL Injection na funcionalidade de busca.
...
\textit{[Esta seção deve ser preenchida pelo autor do projeto, com uma reflexão pessoal sobre o
processo de desenvolvimento. Sugestão de estrutura: (1) principais dificuldades técnicas
encontradas; (2) principais aprendizados, tanto técnicos (Java, Servlets/JSP/JDBC, segurança)
quanto de processo (Aprendizagem Baseada em Projetos, especificação antes de código); (3)
possíveis trabalhos futuros, por exemplo: implementação de autenticação de usuários (o que
reabriria a discussão de controle de acesso da Seção~\ref{sec:seguranca}), paginação da
...
Todo o código-fonte Java do projeto é comentado de duas formas complementares. Primeiro,
...
desde a implementação original de cada funcionalidade. Nesta rodada final de documentação, o
Javadoc foi auditado e complementado nos métodos \texttt{doGet}/\texttt{doPost} de todos os sete
...
parâmetros de requisição e o comportamento em caso de erro. O Javadoc de
\texttt{com.catalogo.model.Filme} segue a recomendação da disciplina de manter um comentário de
...
Um manual do usuário simplificado, voltado a quem nunca utilizou o sistema (linguagem não
técnica), está disponível separadamente em \texttt{docs/manual-usuario.md}. Ele cobre, em uma a
